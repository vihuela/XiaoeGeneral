...
package com.xiaoe.common.retrofit
...
import com.xiaoe.common.ext.ext.pref
import com.xiaoe.common.resource.*
        ...
        if (apiSingletonMap[key] == null) {
            ...
            ExceptionParseMgr.Instance.addCustomerMessageParser { error, e ->
                if (devEnvironment) {
                    ...
                    when (error) {
                        Error.NetWork -> {
                            context.getString(R.string.common_network_netWork)
                        }
                        Error.Internal -> {
                            context.getString(R.string.common_network_internal)
                        }
                        Error.Server -> {
                            context.getString(R.string.common_network_server)
                        }
                        Error.Invalid -> {
                            e.message
                        }
                        else -> {
                            context.getString(R.string.common_network_unKnow)
                        }
                    }
                }
            }
            ...
            ExceptionParseMgr.Instance.addCustomerParser(object : ExceptionParser() {
                override fun handler(e: Throwable?, handler: IHandler): Boolean {
                    if (e != null) {
                        if (XiaoeBusinessException::class.java.isAssignableFrom(e.javaClass)) {
