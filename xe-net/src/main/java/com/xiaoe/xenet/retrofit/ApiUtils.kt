        ...
        if (apiSingletonMap[key] == null) {
            ...
            ExceptionParseMgr.Instance.addCustomerMessageParser { error, e ->
                if (devEnvironment) {
                    ...
                    when (error) {
                        Error.NetWork -> {
                            context.getString(R.string.xent_network_netWork)
                        }
                        Error.Internal -> {
                            context.getString(R.string.xent_network_internal)
                        }
                        Error.Server -> {
                            context.getString(R.string.xent_network_server)
                        }
                        Error.Invalid -> {
                            e.message
                        }
                        else -> {
                            context.getString(R.string.xent_network_unKnow)
                        }
                    }
                }
            }
